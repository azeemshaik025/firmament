//! Binary entrypoint for the Wave 0 runtime scaffold.

use anyhow::Context;
use std::time::Duration;
use tbd_rfq_maker_runtime::{AppConfig, api, bootstrap, tui};
...
async fn main() -> anyhow::Result<()> {
    ...
    let app_state = bootstrap(config)
        .await
        .context("bootstrap runtime scaffold")?;
    ...
    info!(
        ...
        "TBD RFQ maker runtime scaffold ready"
    );
