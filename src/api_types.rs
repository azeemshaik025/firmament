...
/// API integration status for route shells.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum IntegrationStatus {
    /// The API route is wired, but downstream domain workers are not merged yet.
    Placeholder,
}
...
pub struct HtlcAcceptanceTerms {
    ...
    /// True while the HTLC client is not integrated into the route.
...
pub enum RfqResponse {
    /// The route accepted the RFQ shape and produced placeholder quote terms.
    Accepted {
        ...
        /// Integration status for the route shell.
