...
fn project_system_event(event: &SystemEvent) -> StatusRow {
    match event {
        SystemEvent::RuntimeReady { message, .. } => StatusRow::new("System", message, "ready"),
...
fn settlement_receipt_row(
    status: &'static str,
    trade_id: crate::domain::types::TradeId,
    signature: Option<&crate::domain::types::TxSignature>,
) -> StatusRow {
    StatusRow::new(
        "Settlement",
...
fn inventory_status(status: &str) -> &'static str {
    match status {
        "runtime_ready" | "scaffold_ready" | "ready" => "ready",
        "empty" => "empty",
        _ => "status",
    }
}
...
mod tests {
    ...
    fn tui_view_model_projects_quote_events_for_rfq_tab() {
        let quote_id = crate::domain::types::QuoteId::generate();
