...
use crate::{
    ...
    runtime::{RuntimeHandle, RuntimeState},
    tui_state::{
...
pub async fn run(runtime: RuntimeHandle) -> AppResult<()> {
    ...
    let loop_result = run_loop(&mut terminal, runtime).await;
...
async fn run_loop<B: Backend>(terminal: &mut Terminal<B>, runtime: RuntimeHandle) -> AppResult<()> {
    ...
    while !state.should_quit() {
        ...
        if map_io(event::poll(TICK_RATE), "poll terminal input")? {
            ...
            handle_key(key, &mut state, &snapshot);
        }
    }
...
fn handle_key(key: KeyEvent, state: &mut TuiState, runtime: &RuntimeState) {
    let view = TuiViewModel::from_runtime(runtime);
    ...
    let Some(input) = input_from_key(key) else {
        return;
    };
